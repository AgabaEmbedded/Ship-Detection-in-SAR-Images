...
# --- CONFIGURATION ---
Data_dir = r"Input_Data/"  # Update this to your actual data directory
    ...
    with rasterio.open(image_path) as src:
        ...
        for i in range(0, h - patch_size + 1, stride):
            for j in range(0, w - patch_size + 1, stride):
                ...
                # --- QGIS-CONSISTENT RENDERING ---
                # Based on your Symbology: Min=0, Max=0.1, Stretch to MinMax
                vmin, vmax = 0.0, 0.1
                ...
                # 2. Apply the Stretch (The "QGIS Look")
                # Clip values to the 0-0.1 range
                patch_display = np.clip(patch_display, vmin, vmax)
                
                # 3. Scale to 8-bit (0-255)
                # Formula: (val - 0) / (0.1 - 0) * 255 -> val * 2550
                patch_display = (patch_display - vmin) / (vmax - vmin) * 255
